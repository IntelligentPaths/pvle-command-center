...
- **Allowlist is currently these three emails:** `wamer27@gmail.com`, `info@intelligentpaths.com`, `Juleon@livepuravida.org`. To change, edit `ALLOWED_EMAILS` in Vercel + redeploy.
...
- **Enrollment meters** — the "Founding cohort" (Sankofa) and "After-school" (PVY) meters are labeled placeholders; wire real numbers once enrollment/interest counts live in the Sheet (e.g. a `Leads` roll-up).
